...
if(!function_exists("stm_getCurrencySelectorHtml")) {
    function stm_getCurrencySelectorHtml() {
        $showCurrencySelect = stm_me_get_wpcfto_mod('top_bar_currency_enable', false);
        $multipleCurrencyList = stm_me_get_wpcfto_mod("currency_list", "");
        ...
        if($multipleCurrencyList != null) {
			...
			if($multipleCurrencyList != null) {
				foreach ($multipleCurrencyList as $k => $val) {
            ...
            if(count($currency) > 1 && $showCurrencySelect) {
