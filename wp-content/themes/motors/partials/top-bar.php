        ...
        <div class="container">
            ...
            <div class="clearfix top-bar-wrapper">
				...
				<?php if ( $top_bar_wpml_switcher ): ?>
					<?php if ( !empty( $langs ) ): ?>
                        ...
                        <div class="pull-left language-switcher-unit">
                            <div class="stm_current_language <?php echo esc_attr( $langs_exist ); ?>" <?php if ( count( $langs ) > 1 || is_author() ) { ?> id="lang_dropdown" data-toggle="dropdown" <?php } ?>>
								...
								<?php endif; ?>
								<?php echo esc_attr( ICL_LANGUAGE_NAME ); ?><?php if ( count( $langs ) > 1 || is_author() ) { ?>
                                ...
                                <ul class="dropdown-menu lang_dropdown_menu" role="menu"
									...
									<?php foreach ( icl_get_languages( 'skip_missing=0' ) as $val ) : ?>
										...
										if ( !$val['active'] ):
                                            ...
                                            <li role="presentation">
                                                <a role="menuitem" tabindex="-1"
                                                   href="<?php echo esc_url( $mainUrl . $request_uri ); ?>">
					...
					<?php endif; ?>
				<?php endif; ?>
				<?php if ( stm_me_get_wpcfto_mod( 'top_bar_currency_enable', false ) ) stm_getCurrencySelectorHtml(); ?>
                    ...
                    <div class="pull-right xs-pull-left top-bar-info-wrap">
                        <ul class="top-bar-info clearfix">
							<?php if ( $top_bar_working_hours ) { ?>
                                <li <?php if ( !$top_bar_working_hours_mobile ){ ?>class="hidden-info"<?php } ?>><?php echo stm_me_get_wpcfto_icon( 'top_bar_working_hours_icon', 'fa fa-clock-o' ); ?> <?php stm_dynamic_string_translation_e( 'Top Bar Working Hours Label', $top_bar_working_hours ); ?></li>
							<?php } ?>
							<?php if ( $top_bar_address ) { ?>
								<?php $header_address_url = stm_me_get_wpcfto_mod( 'header_address_url' ); ?>
                                <li <?php if ( !$top_bar_address_mobile ){ ?>class="hidden-info"<?php } ?>>
                                    <span id="top-bar-address" class="fancy-iframe" data-iframe="true"
                                          data-src="<?php echo esc_attr( $header_address_url ); ?>">
							...
							<?php if ( $top_bar_phone ) { ?>
                                <li <?php if ( !$top_bar_phone_mobile ){ ?>class="hidden-info"<?php } ?>><?php echo stm_me_get_wpcfto_icon( 'top_bar_phone_icon', 'fa fa-phone' ); ?>
