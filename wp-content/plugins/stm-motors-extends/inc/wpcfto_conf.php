...
function stm_me_motors_get_wpcfto_mod( $opt_name, $default = '' )
{
...
function stm_me_get_wpcfto_icon( $icon_opt, $defaultIcoClass, $additionalClass = '' )
{
	$iconArr = stm_me_motors_get_wpcfto_mod( $icon_opt, false );
	
	$tbAddrIcon = $defaultIcoClass;
	$tbIconStyle = '';
	if ( $iconArr && !empty($iconArr['icon'])) {
		$tbAddrIcon = $iconArr['icon'];
		$tbIconStyle = 'color: ' . $iconArr['color'] . '; font-size: ' . $iconArr['size'] . 'px;';
	}
	
	return '<i class="' . esc_attr( $additionalClass . ' ' . $tbAddrIcon ) . '" style="' . esc_attr( $tbIconStyle ) . '"></i>';
}
...
{
	$logo_main = stm_me_motors_get_wpcfto_mod( $opt_name, $default );
...
function stm_me_get_header_layout()
{
	...
	return stm_me_motors_get_wpcfto_mod( 'header_layout', $defaultHeader );
}
...
function stm_me_wpcfto_custom_icons( $iconsCollect )
{
	...
	if ( class_exists( 'STM_Custom_Icons' ) ) {
		...
		if ( file_exists( $font_configs['stm-icon']['include'] . '/' . $font_configs['stm-icon']['config'] ) ) {
			require_once $font_configs['stm-icon']['include'] . '/' . $font_configs['stm-icon']['config'];
			
			foreach ( $icons as $k => $val ) {
				foreach ( $val as $key => $item ) {
					$iconsCollect[] = array( 'title' => $k . '-' . $item['class'], 'searchTerms' => array( $item['class'] ) );
				}
			}
		}
	}
