...
<head>
    ...
    <?php if(get_theme_mod('listing_archive', '') == get_the_ID() || get_theme_mod('rental_datepick', '') == get_the_ID() || (function_exists( 'is_shop' ) && is_shop()) || is_category() ) :
        $pageId = get_the_ID();
        if((function_exists( 'is_shop' ) && is_shop())) $pageId = get_option( 'woocommerce_shop_page_id' );
		if ( is_category() ) $pageId = get_option( 'page_for_posts' );
        ?>
        <link rel="canonical" href="<?php echo get_the_permalink($pageId); ?>" />
    <?php endif; ?>
...
<?php $body_custom_image = stm_me_get_wpcfto_img_src( 'custom_bg_image', '' ); ?>
