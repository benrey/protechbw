...
<div class="stm-single-car-links">
	<?php foreach($links as $icon => $link): ?>
		...
		<div class="stm-single-car-link unit-<?php echo esc_attr($icon); ?> heading-font">
			<a href="<?php echo esc_url($link['link']) ?>" target="<?php echo esc_attr($target); ?>" <?php if(!empty($link['modal'])) echo stm_do_lmth($link['modal']) . ' data-id ="' . $id . '" data-title="' . stm_generate_title_from_slugs($id, false) . '" class="stm-modal-action"'; ?>>
