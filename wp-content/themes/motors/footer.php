...
if ( !stm_is_auto_parts() && !stm_is_rental() ) :
    if ( is_singular( stm_listings_post_type() ) ) {
        if ( stm_me_get_wpcfto_mod( 'show_calculator', false ) ) get_template_part( 'partials/modals/car-calculator' );
        if ( stm_me_get_wpcfto_mod( 'show_offer_price', false ) ) get_template_part( 'partials/modals/trade-offer' );
        if ( stm_me_get_wpcfto_mod( 'show_trade_in', false ) ) get_template_part( 'partials/modals/trade-in' );
    }
    ...
    if ( stm_me_get_wpcfto_mod( 'show_test_drive', false ) ) get_template_part( 'partials/modals/test-drive' );
    get_template_part( 'partials/modals/get-car-price' );

    $show_compare = ( is_single( get_the_ID() ) ) ? stm_me_get_wpcfto_mod( 'show_listing_compare', false ) : stm_me_get_wpcfto_mod( 'show_compare', false );
